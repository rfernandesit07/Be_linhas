...
if (class_exists('Plugs99_Plugin_Updater') && function_exists('p99_fetchkey')) {
    add_action('init', function() {
        ...
        $key = p99_fetchkey('Astra WP Portfolio');
        new Plugs99_Plugin_Updater('https://99plugs.com/', PMAIN_4278, [
            'version'   => "1.13.3",
            'license'   => $key,
            'item_id'   => "4278",
...
foreach (['pre_set_site_transient_update_plugins', 'pre_site_transient_update_plugins', 'site_transient_update_plugins'] as $hook) {
  add_filter($hook, function ($transient) {
    if (isset($transient) && is_object($transient) && isset($transient->response[plugin_basename(PMAIN_4278)])) {
      if (strpos($transient->response[plugin_basename(PMAIN_4278)]->package, '99plugs') === false) {
        unset($transient->response[plugin_basename(PMAIN_4278)]);
      }
    }
    return $transient;
  });
...
add_filter('http_request_args', function ($r, $url) {
    if (strpos($url, 'https://api.wordpress.org/plugins/update-check/1.1/') === 0) {
        ...
        unset($plugins['plugins'][plugin_basename(PMAIN_4278)]);
        $r['body']['plugins'] = json_encode($plugins);
    }
    return $r;
}, PHP_INT_MAX, 2);

register_activation_hook(PMAIN_4278, function() {
    $apikeys = get_option('apikeys', []);
    $keyname = 'Astra WP Portfolio';
